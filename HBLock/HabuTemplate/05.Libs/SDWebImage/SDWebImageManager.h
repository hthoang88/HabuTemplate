...
typedef enum
{
    ...
    /**
     * TaiT:
     * set NSOperationQueuePriority of download operation is highest
     */
    SDWebImageHighestPriority = 1 << 5
} SDWebImageOptions;

typedef void(^SDWebImageCompletedBlock)(UIImage *image, NSError *error, SDImageCacheType cacheType);
typedef void(^SDWebImageCompletedWithFinishedBlock)(NSURL *requestURL, UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished);
...
// additional delegate (use for old version)
...
// additional function (user for old version)
- (UIImage *)imageWithURL:(NSURL *)url; // get image from memory cache
