...
@synthesize reduceHeight = _reduceHeight;
@synthesize hideLastUpdateLabel;
...
- (void)layoutSubviews {
    ...
    if (_reduceHeight > 0){
        titleFrame.origin.y=-_reduceHeight/3;
    }
...
- (SVPullToRefreshArrow *)arrow {
    if(!arrow && pullToRefreshActionHandler) {
        if (_reduceHeight > 0) {
            self.arrow = [[SVPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, 6 - _reduceHeight, 22, 48)];
        } else {
...
static NSDateFormatter *dateFormatStatic;

- (NSDateFormatter *)dateFormatter {
    if(!dateFormatter) {
        if (!dateFormatStatic) {
            dateFormatStatic = [[NSDateFormatter alloc] init];
        }
        dateFormatter = dateFormatStatic;
...
- (void)setPullToRefreshActionHandler:(void (^)(void))actionHandler {
    ...
    titleLabel.text = localizedString(@"Pull to refresh...");
    ...
    if (_reduceHeight > 0) {
        self.frame = CGRectMake(0, -SVPullToRefreshViewHeight + _reduceHeight, self.scrollView.bounds.size.width, SVPullToRefreshViewHeight - 40);
    } else {
...
- (void)startAnimating{
    ...
    titleLabel.text = localizedString(@"Loading...");
...
- (void)stopAnimating {
    self.state = SVPullToRefreshStateHidden;
    //Show current date time for last update date text
    if (!self.hideLastUpdateLabel)
    ...
    if(pullToRefreshActionHandler) {
        switch (newState) {
            ...
            case SVPullToRefreshStateTriggered:
                titleLabel.text = localizedString(@"Release to refresh...");
...
- (void)addReduceHeight:(CGFloat)reduce
{
    self.pullToRefreshView.reduceHeight = reduce;
}
